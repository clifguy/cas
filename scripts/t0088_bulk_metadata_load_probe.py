#!/usr/bin/env python3
"""Acceptance benchmark: batched vs sequential metadata-patch calls.

Compares wall-clock for two paths that apply a tag-add metadata patch
to N documents via the consolidated ``update_metadata`` MCP tool
(CAS-ADR-029 v4 plural-noun convention):

1. N sequential ``update_metadata`` MCP calls, each with a length-1
   ``items`` collection (baseline).
2. One ``update_metadata`` MCP call with N items (batched).
...
async def _run_sequential_fastmcp(vault_id: str, doc_ids: list[str]) -> float:
    ...
    length-1 ``update_metadata`` invocation.
    """
    mcp = mcp_server.mcp
    ...
    for doc_id in doc_ids:
        raw = await mcp.call_tool(
            ...
            {
                "vault_id": vault_id,
                "items": [{"document_id": doc_id, "tags": {"add": ["bulk_test"]}}],
            },
        )
...
async def _run_bulk_fastmcp(vault_id: str, doc_ids: list[str]) -> float:
    ...
    start = time.perf_counter()
    raw = await mcp.call_tool("update_metadata", {"vault_id": vault_id, "items": items})
...
async def _run_bulk_stdio(session: ClientSession, vault_id: str, doc_ids: list[str]) -> float:
    items = [{"document_id": d, "tags": {"add": ["bulk_test"]}} for d in doc_ids]
    start = time.perf_counter()
    resp = await session.call_tool("update_metadata", {"vault_id": vault_id, "items": items})
    elapsed = time.perf_counter() - start
    payload = _parse_call_tool_response(resp)
    if "error" in payload and "results" not in payload:
        raise RuntimeError(f"bulk call failed: {payload!r}")
