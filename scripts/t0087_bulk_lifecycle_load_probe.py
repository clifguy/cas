#!/usr/bin/env python3
"""T-0087 acceptance benchmark: sage_bulk_set_lifecycle vs sequential calls.

Compares wall-clock for two paths that flip N documents through a
lifecycle transition:

1. N sequential ``sage_set_lifecycle`` MCP calls (baseline).
2. One ``sage_bulk_set_lifecycle`` MCP call with N items.
...
- ``--mode stdio`` (default): launches ``python -m sage.mcp_server`` as
  ...
  T-0087 acceptance measurement; the ticket gate is calibrated against
  it.
...
Acceptance gate (per the T-0087 ticket): the bulk wall-clock must be
...
Usage:
    ...
    SAGE_TEST_STUB_PROVIDERS=1 .venv/bin/python \\
        -m scripts.t0087_bulk_lifecycle_load_probe --n 500
"""
...
def _vault_config_dict(vault_id: str, brain_root: Path, storage_root: Path) -> dict:
    return {
        "vault": {
            "id": vault_id,
            "name": f"T-0087 Probe Vault {vault_id}",
            "owner": "benchmark",
            "visibility": "personal",
...
async def _run_bulk_fastmcp(vault_id: str, doc_ids: list[str]) -> float:
    """Single in-process FastMCP.call_tool dispatch for sage_bulk_set_lifecycle."""
    mcp = mcp_server.mcp
    items = [{"document_id": d, "action": "archive"} for d in doc_ids]
    start = time.perf_counter()
    raw = await mcp.call_tool("sage_bulk_set_lifecycle", {"vault_id": vault_id, "items": items})
...
async def _run_bulk_stdio(session: ClientSession, vault_id: str, doc_ids: list[str]) -> float:
    ...
    start = time.perf_counter()
    resp = await session.call_tool(
        "sage_bulk_set_lifecycle", {"vault_id": vault_id, "items": items}
    )
    elapsed = time.perf_counter() - start
    payload = _parse_call_tool_response(resp)
    if "error" in payload:
...
async def _bench_fastmcp(n: int) -> tuple[float, float]:
    ...
    with tempfile.TemporaryDirectory(prefix="t0087_probe_") as tmp:
        tmp_path = Path(tmp)
        brain_a = tmp_path / "brain_seq"
        store_a = tmp_path / "storage_seq"
        brain_a.mkdir()
        store_a.mkdir()
        # Each path gets a fresh vault so it starts with N active documents
        # — re-using the same one would land path 2 in an invalid-transition
        # fest (everything's already archived).
        vault_a, ids_a, svc_a = await _seed_vault_inprocess("t0087_probe_seq", brain_a, store_a, n)
        try:
        ...
        brain_b.mkdir()
        store_b.mkdir()
        vault_b, ids_b, svc_b = await _seed_vault_inprocess("t0087_probe_bulk", brain_b, store_b, n)
        try:
...
async def _bench_stdio(n: int) -> tuple[float, float]:
    ...
    with tempfile.TemporaryDirectory(prefix="t0087_probe_stdio_") as tmp:
        ...
        vault_seq, ids_seq = await _build_vault_on_disk(vault_root, "t0087_probe_seq", n)
        vault_bulk, ids_bulk = await _build_vault_on_disk(vault_root, "t0087_probe_bulk", n)
