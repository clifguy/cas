...
The maintenance tools' retired names remain working aliases with no scheduled removal: the path `/mcp_admin` serves the identical roster, and each `admin_*` or `maint_*` tool name dispatches as its bare counterpart (for example `get_vault_config`) on whichever mount registers that counterpart. New configurations should use the bare names.
