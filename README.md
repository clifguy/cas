...
The maintenance tools' retired names remain working aliases with no scheduled removal: the path `/mcp_admin` serves the identical roster, and each `admin_*` or `maint_*` tool name sent on the wire dispatches as its bare counterpart (for example `get_vault_config`) on whichever mount registers that counterpart. The alias protects callers that name tools by string — raw JSON-RPC clients, scripts, configurations — and cannot reach a catalog-validating client such as Claude Code, which resolves a tool name against `tools/list` before any request leaves the process; a retired name in such a client's configuration or permission allowlist must be updated. New configurations should use the bare names.
