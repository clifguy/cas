...
def rehearse(
    ...
    *,
    ...
    if action not in ("rehearse", "cleanup-rehearsal") or not re.fullmatch(r"r[0-9]{1,20}", run_id):
        raise ValueError("invalid rehearsal action or run identity")
    resource_group = az("group", "show", "--name", group)
    if (resource_group.get("tags") or {}).get("casPostgresMigration"):
        raise ValueError("rehearsal requires an empty migration fence")
    deployment, bootstrap, bootstrap_env = coordinates(az, environment, group, generation)
    source = output(deployment, "postgresServerFqdn")
    target = output(replacement(az, group, generation), "postgresServerFqdn")
    check_servers(az, group, source, target)
    job = f"job-pg-migration-{environment}"
    assert_jobs_idle(
        az,
        group,
        (output(deployment, "bootstrapJobName"), output(deployment, "maintenanceJobName"), job),
        "another database job is active",
    )
    configured = az("containerapp", "job", "show", "--resource-group", group, "--name", job)
    ...
    if (
        any(env.get(k) != v for k, v in expected.items())
        or container["image"] != image
        or configured["identity"] != bootstrap["identity"]
        or container.get("resources") != {"cpu": 1, "memory": "2Gi"}
        or configured["properties"]["configuration"]["replicaTimeout"] != 7200
    ):
        raise ValueError(
            "prepared rehearsal job has stale identity, image, coordinates or resources"
        )
    az(
        "containerapp",
        "job",
    ...
    execution = az("containerapp", "job", "start", "--resource-group", group, "--name", job)["name"]
    # Persist the handle before waiting. On cancellation, absence of a result is not success.
    if os.environ.get("GITHUB_ACTIONS") == "true":
        Path("rehearsal-execution.json").write_text(
            json.dumps(
                {
                    "job": job,
                    "execution": execution,
...
def collect_rehearsal_report(
    az: Azure, environment: str, *, sleep: Callable[[float], None] = time.sleep
) -> None:
    record = json.loads(Path("rehearsal-execution.json").read_text())
    # Only identifiers generated by Azure/the validated workflow enter KQL.
    if not all(re.fullmatch(r"[a-zA-Z0-9-]+", record[k]) for k in ("job", "execution", "run_id")):
        raise ValueError("invalid rehearsal evidence identity")
    deployment = az("deployment", "sub", "show", "--name", environment)
    workspace = output(deployment, "logAnalyticsCustomerId")
    prefix = "CAS_REHEARSAL_REPORT "
    query = (
        f"ContainerAppConsoleLogs_CL | where ContainerJobName_s == '{record['job']}' "
        f"| where ContainerGroupName_s startswith '{record['execution']}-' "
        f"| where Log_s startswith '{prefix}' "
        f"| extend report = parse_json(substring(Log_s, {len(prefix)})) "
        f"| where tostring(report.run_id) == '{record['run_id']}' "
        "| project report=tostring(report)"
    )
    for attempt in range(11):
        ...
        if rows:
            ...
            if len(reports) != 1:
                raise ValueError("ambiguous rehearsal reports")
            report = json.loads(reports.pop())
            Path("rehearsal-report.json").write_text(
                json.dumps({"execution": record, "report": report}, indent=2)
            )
            expected = (
                "rehearsal_verified" if record["action"] == "rehearse" else "rehearsal_cleaned"
            )
            if report.get("status") != expected or (
                record["action"] == "rehearse" and report.get("image") != record["image"]
            ):
                raise ValueError("rehearsal report does not establish success")
            return
        if attempt < 10:
            sleep(30)
    raise ValueError(
        "rehearsal report unavailable after bounded log-ingestion wait; retain execution handle"
    )
...
def main() -> None:
    ...
    if action == "collect-report":
        collect_rehearsal_report(azure, environment)
        return
    if action in ("rehearse", "cleanup-rehearsal"):
        run_id = os.environ.get("REHEARSAL_RUN_ID", "")
        if not run_id:
            if action == "cleanup-rehearsal":
                raise ValueError("cleanup requires the original rehearsal run identity")
            run_id = "r" + os.environ["GITHUB_RUN_ID"]
        rehearse(azure, environment, group, generation, run_id, action)
        return
    if action == "prepare":
