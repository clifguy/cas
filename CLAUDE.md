...
Storage is a per-profile backend choice (CAS-ADR-042): an **embedded** backend (SQLite graph store + LanceDB vector/full-text content) and a **Postgres** backend (with pgvector). The committed `sage/config.yaml` selects `postgres`. SAGE runs under a **local** or **cloud** profile; the cloud profile targets Azure Container Apps / APIM / Entra ID / Key Vault / Postgres. Infrastructure-as-code is in `infra/` (Bicep); container images and deploy scripts in `deploy/`.
