...
Section numbering is the skill's own and is not the failure-log numbering. Several sections are anchored to failure records that carry a different number (F4's section cites records F4 and F6; G1's cites records F44, F45, and F49). Where a section names a record, it says "recorded as" to mark the reference. A section's anchor set is likewise independent of its failure class's record set: a class is a claim about what went wrong, an anchor is a worked example of a shape, and a case can be one without being the other. G1 carries a live instance and states it.
...
- If yes, does the field/parameter use a typed alias from `sage/models/schemas.py` — `DocumentIdStr`, `EdgeIdStr`, `Sha256Str`, `DocumentDateStr`, `VaultIdStr`, `UserIdStr`, or another existing alias — rather than the bare type (`str`, `str | None`)?
...
**What this catches.** A test or gate that passes identically against the correct implementation and the defective one, so it cannot go red while the condition it exists to enforce is breached. Two documented cases, from opposite directions. In the first (recorded as F44), a test asserted that a directive string appeared in a constructed model prompt — true whether or not the model obeyed it, because model behaviour was never in the assertion's causal path; the constraint was breached for eleven weeks with the gate green throughout. In the second (recorded as F45), a test asserted that a failed capability lookup degrades to an unchecked call and warns once — true of both the correct implementation and the defective one, because the fake client's only affordance was an error that never cleared, so "a condition that clears between the first call and the second" could not be expressed at the fixture at all. A third case (recorded as F49) sits on the setup side rather than the assertion side: a test's config block was presented as an anti-coincidental control, yet no assertion read anything it produced — deleting the whole block left the test passing, and no mutation of the code under test could ever have revealed that.

**On F45, and on section anchors generally.** The `gate_integrity_gap` failure class does not claim F45, and that is not a disagreement to reconcile — the two answer different questions. A class asks whether *the verification is itself the failure*, which is a claim about what went wrong; F45's primary property is a plain ordering defect that a weak test happened to miss, so the class excludes it. A section anchor asks whether the case *teaches the shape a reviewer has to recognize*; F45's test was non-discriminating in exactly the way this section describes, it is the case that forced this section's procedural form, and the reference implementation at the end of this section is drawn from its fix. It earns its place here on the second question while failing the first. Read a section anchor as a worked example, never as a class roster.
...
**Scope: any verification in the diff, not only tests.** These prompts apply to every artifact in the diff whose job is to go red — a test, a drift or conformance check that runs on a schedule, a CI assertion, a structural gate helper. A comparison inside a checker that reports divergence is a verification even though no test framework runs it, and it can be non-discriminating in the same way (recorded as F61, whose defect is in a scheduled drift check rather than in a test). Where a prompt below says "test", read "verification".

**What has been measured, and one axis that resisted it.** This section has been replayed blind against its anchors — a fresh reviewer given only this file and one commit, with nothing else in scope. It reaches F65's introducing commit, naming the fixtures that still report with the rule under test disabled. It does not reach F61's, and two attempts to reword the prompts below so that it would — one adding a required field naming the input property every fixture holds constant, one making the rival field derive from that property — changed nothing, on either candidate anchor or on either model tried. The axis is real and the records name it: in F50, F56, F60, F61 and F63 the rival differed from the correct code only on an input none of the fixtures carried, and a search run over implementations cannot reach it. What has not been shown is that a prompt closes it. The passes themselves — fixtures, arms, and outcomes — are recorded in this skill's `tooling_entry` in the cas vault, which is where a later round should start rather than re-deriving them. So treat a clean walk of this section as a floor rather than a clearance — in every one of those records the independent PR-stage review found the defect on a surface this gate had already cleared, and its advantage, not inheriting the author's framing, is not something a prompt here can supply.
...
- For each test ADDED in the diff: is every setup block consumed by an assertion? Name any fixture, config, or construction whose deletion would leave the test passing (recorded as F49). Inert setup reads as a control and will be described as one in the docstring, so a reviewer who trusts the docstring inherits the error rather than catching it. Not reachable by the rival-implementation prompts above — breaking the implementation says nothing about scaffolding that was never in any assertion's causal path; the discriminating mutation is deleting the setup itself.
