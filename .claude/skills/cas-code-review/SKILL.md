...
- **Logic placement.** For any diff touching an MCP tool body (`sage/sage_api_tools.py`, `sage/app_tools.py`) or a REST router body, check whether it adds branching, semantic validation, defaulting, or refusal there rather than in `sage/services/`. Boundary shape validation through a typed alias (F3's patterns) is not this finding. Anything else is a parity defect unless it is a CAS-ADR-052 divergence recorded in the *SAGE MCP Tool Surface* steering document's "Surface divergences" register. The parity tests cannot see it when both sides are edited at once (F155); the import-linter contract catches only a tool that bypasses the services, not logic written inline.
