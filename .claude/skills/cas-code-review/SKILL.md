...
Section numbering is the skill's own and is not the failure-log numbering. Several sections are anchored to failure records that carry a different number (F4's section cites records F4 and F6; G1's cites records F44, F45, and F49). Where a section names a record, it says "recorded as" to mark the reference.
