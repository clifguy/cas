...
a name alone does not satisfy the dependency. The selected SAGE capability is an
explicit external dependency and must be resolved through shared project-policy.

Resolve only authorities applicable to the selected operation and effects. Read
CAS Invocation Authority when establishing an action grant; PR Review Conventions
for independent review/disposition; Release Classification for release-impact
judgment; Ticket Conventions and the named methodology for cohort/ticket work;
Cloud Deployment Discipline for deploy or deployment-surface review. Retrieve a
needed live authority from the correct SAGE deployment and follow supersession
...
execution/browser/collaboration tools from the intended environment. Capability
absence is a blocked prerequisite, not a successful no-op. A direct Azure review
resolves the existing commit operation and explicitly requires cas-azure-review;
there is no new azure-review resolver operation. Apply structural triggers to both
rename paths and inspect runtime dependency additions in ordinary source semantically.

Gate-status measurement is retired; no exposure counter, threshold, per-pass
