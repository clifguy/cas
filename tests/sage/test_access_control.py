...
async def test_bh_009_vault_owner_auto_registered(graph_store, user_service):
    ...
    assert owner.type == UserType.HUMAN
    ...
    assert users[0].type == UserType.HUMAN
...
# SAGE's register_user endpoint accepts type=agent correctly (BH-011
...
async def test_bh_011_register_user_agent(user_service):
    ...
    request = RegisterUserRequest(display_name="test_agent", type=UserType.AGENT)
    user = await user_service.register_user(request)

    assert user.display_name == "test_agent"
    assert user.type == UserType.AGENT
