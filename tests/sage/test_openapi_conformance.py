...
# ---------------------------------------------------------------------------
# Test 2c: the store-refusal codes are declared in pairs
# ---------------------------------------------------------------------------
...
def test_store_refusal_operations_each_declare_both_not_one(live_openapi: dict):
    ...
    The two statuses answer one question between them -- whether waiting could
    change the store's answer -- so an operation that names only the 502 tells
    a caller its refusals are never transient, which no service path promises.
    Declaring one is therefore a defect rather than a narrower contract.

    Anti-coincidental-pass: asserted over the live spec rather than a roster,
    so an operation added with a lone status is caught without an edit here.
    """
    lopsided = [
