...
async def test_cr_037_debug_false_produces_no_resolution_path(
...
async def test_cr_038_debug_records_anchor_hit_on_surfaced_edge(
    graph_store, graph_ops_service
):
    ...
    hits = [
        e for e in out.resolution_path
        if e.event_type == "anchor_hit" and e.edge_id == covers_id
    ]
...
async def test_cr_039_debug_records_anchor_miss_on_suppressed_edge(
    graph_store, graph_ops_service
):
    ...
    misses = [
        e for e in out.resolution_path
        if e.event_type == "anchor_miss" and e.edge_id == bad_edge.id
    ]
    assert len(misses) == 1
...
async def test_cr_040_debug_records_retracts_applied(
    graph_store, graph_ops_service
):
...
async def test_cr_041_debug_records_tombstone_applied(
    graph_store, graph_ops_service
):
...
async def test_cr_042_resolution_path_preserves_event_order(
    graph_store, graph_ops_service
):
    ...
    ac_events = [
        e for e in out.resolution_path if e.edge_id == covers_ac.id
    ]
    assert any(e.event_type == "retracts_applied" for e in ac_events)
    retract_idx = next(
        i for i, e in enumerate(ac_events)
        if e.event_type == "retracts_applied"
    )
    ...
    anchor_indices = [
        i for i, e in enumerate(out.resolution_path)
        if e.event_type in ("anchor_hit", "anchor_miss")
    ]
    retract_indices = [
        i for i, e in enumerate(out.resolution_path)
