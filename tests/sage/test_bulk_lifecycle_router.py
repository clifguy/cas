...
POST /sage_vaults/{vault_id}/lifecycles.
"""
...
async def test_bulk_endpoint_happy_path_returns_200_and_response_envelope(seeded_app):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(f"/sage_vaults/{vault_id}/lifecycles", json=body)
...
async def test_bulk_endpoint_unknown_vault_returns_404(seeded_app):
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/sage_vaults/ghost/lifecycles", json=body)
...
async def seeded_six_app(minimal_vault_config_dict, monkeypatch):
    ...
    for doc_id in seeded_ids:
        await graph_store.insert_document(_make_doc(doc_id))
        await graph_store.update_document(doc_id, {"semantic_abstract": _BLOAT_PROBE_ABSTRACT})
...
async def test_http_error_envelope_intact_in_light_mode(seeded_six_app):
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        light_resp = await client.post(f"/sage_vaults/{vault_id}/lifecycles", json=light_body)
        full_resp = await client.post(f"/sage_vaults/{vault_id}/lifecycles", json=full_body)
