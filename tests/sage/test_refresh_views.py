...
async def test_bh043_both_view_dimensions_generated(
    graph_store, utilities_service, minimal_config, tmp_vault_dir,
):
    ...
    await _insert_test_document(
        graph_store, "doc_a", "Doc A", "patents/doc_a.md",
...
async def test_bh044_symlinks_point_to_source_files(
    graph_store, utilities_service, minimal_config,
):
    ...
    await _insert_test_document(
        graph_store, "doc_cs", "Claim Set", "patents/claim_set.md",
        doc_type="patent",
    )
...
async def test_bh045_full_regeneration(
    ...
    await _insert_test_document(
        graph_store, "doc_a", "Doc A", "test/doc_a.md",
        lifecycle_status="active", doc_type="note",
    )
...
async def test_bh046_failed_pipeline_documents_in_views(
    ...
    await _insert_test_document(
        graph_store, "doc_fail", "Failed Doc", "test/failed.md",
        lifecycle_status="active", doc_type="patent",
...
async def test_bh047_empty_categories_no_directory(
    ...
    await _insert_test_document(
        graph_store, "doc_only", "Only Doc", "test/only.md",
        lifecycle_status="active", doc_type="patent",
    )
...
async def test_bh048_null_doc_type_excluded(
    ...
    await _insert_test_document(
        graph_store, "doc_untyped", "Untyped Doc", "test/untyped.md",
...
async def test_empty_vault_produces_no_views(
    utilities_service, minimal_config,
):
...
async def test_filename_collision_handled(
    ...
    await _insert_test_document(
        graph_store, "doc_v1", "Spec V1", "v1/spec.md",
        lifecycle_status="active", doc_type="patent",
    )
    await _insert_test_document(
        graph_store, "doc_v2", "Spec V2", "v2/spec.md",
        lifecycle_status="active", doc_type="patent",
    )
