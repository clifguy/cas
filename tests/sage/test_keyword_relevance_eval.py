...
# --- slice pinning (script-level, pure) ------------------------------------


def test_pin_gold_targets_into_slice() -> None:
    """A pinned target is always selected, even when the sampler omits it.

    We pick a target the unpinned ``_select_slice`` did not choose for this
    (seed, size), then prove ``pin_and_select`` forces it in while keeping the
    slice size-bounded and duplicate-free.
    """
    from scripts.eval_keyword_fidelity import _select_slice, pin_and_select

    candidates = [(f"d{i}", "adr") for i in range(8)]
    seed, size = 7, 3

    base = _select_slice(candidates, size, seed)
    assert len(base) == size
    excluded = next(doc_id for doc_id, _ in candidates if doc_id not in base)

    pinned = pin_and_select(candidates, [excluded], size, seed)

    assert excluded in pinned  # forced in despite the sampler omitting it
    assert len(pinned) == size  # still bounded
    assert len(set(pinned)) == len(pinned)  # no duplicates


# --- gold selector resolution (script-level) -------------------------------


def _doc(doc_id: str, path: str, status: str, adr: str | None = None):  # noqa: ANN202
    ...
    return SimpleNamespace(
        ...
        tier3_metadata=({"adr_id": adr} if adr else None),
    )


def test_resolve_gold_prefers_active_and_handles_both_selector_forms() -> None:
    """``adr:<id>`` resolves to the active ADR; a source_path resolves directly."""
    from scripts.eval_keyword_fidelity import _resolve_gold

    docs = [
        _doc("adr042_old", "imports/cas-adr-042_v1.md", "archived", adr="042"),
        _doc("adr042_active", "imports/cas-adr-042_v2.md", "active", adr="042"),
        _doc("steer_active", "imports/conventions_v3.md", "active"),
    ]
    gold = [
        GoldQuery(query="CAS-ADR-042", primary_id="adr:042", relevant_ids=[]),
        GoldQuery(
            query="conventions",
            primary_id="imports/conventions_v3.md",
            relevant_ids=["adr:042"],
        ),
    ]

    resolved = _resolve_gold(docs, gold)

    # adr:042 picks the ACTIVE version, never the archived one
    assert resolved[0].primary_id == "adr042_active"
    assert resolved[1].primary_id == "steer_active"
    assert resolved[1].relevant_ids == ["adr042_active"]


def test_resolve_gold_raises_on_unresolved_selector() -> None:
    """An unresolvable selector fails loudly rather than scoring against nothing."""
    from scripts.eval_keyword_fidelity import _resolve_gold

    docs = [_doc("x", "imports/x.md", "active")]
    with pytest.raises(ValueError):
        _resolve_gold(docs, [GoldQuery(query="q", primary_id="adr:999", relevant_ids=[])])
