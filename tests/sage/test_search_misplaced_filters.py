...
#: One well-formed value of every JSON shape. An annotation that cannot hold
#: one rejects it at the framework's argument model -- before the guard runs
...
def test_published_tripwires_are_marked_as_tripwires():
    ...
    for key in _SEARCH_FILTER_KEYS:
        ...
        assert props[key] == expected, (
            f"search.{key} must be published untyped, as {expected!r}; got "
            f"{props[key]!r}. A tripwire is never consumed, so shape "
...
async def test_every_tripwire_refuses_every_json_shape(vault_services, key, value):
    ...
    string, list, object and boolean rejected at the framework's argument
    model, before the guard ran, so this test fails against that
    annotation. It also fails against a wrap validator that narrows the
    tripwire without changing its published schema, which the schema pin
    above cannot see.
    """
    result = await mcp.call_tool(
        "search",
        {"vault_id": "test_vault", "mode": "catalog", key: value},
    )
    envelope = _decode_envelope(result)
    assert envelope["error"] == "misplaced_filters", (
