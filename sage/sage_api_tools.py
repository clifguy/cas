...
def register_sage_tools(
    ...
    async def migrate_vault(vault_id: str) -> dict:
        ...
        log names each one. A document indexed before the text above its first
        ...
        source changed since it was indexed or cannot be read, or with pipeline
        work in flight, is left for a later call, and the server log names each
        one. And every passage gains its
