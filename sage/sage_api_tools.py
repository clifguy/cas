    ...
    async def read_projection(
        ...
        Args:
            ...
            write_to_path: Absolute filesystem path, resolved on the
                ...
                text to this path and returns metadata only; the target must
                not exist, and its parent must exist and be writable. Where
                it does not, the response is a download recipe carrying this
                path for the caller's own environment to write. The path must
                be absolute either way, and is checked before the projection
                is read, so a malformed path reports ``write_path_invalid``
                whatever the document's pipeline state.
