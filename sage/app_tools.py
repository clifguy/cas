...
_FILE_ENTRIES = published_item_list(
    ...
    description=(
        ...
        "caller's machine is the machine running the SAGE server process, or "
        ...
        "after issue by default, and every leg's byte delivery plus the "
        "completion call must finish inside that window; the recipe's own "
        "``expires_at`` is authoritative. A leg's token is also reclaimed once "
        "3 refused deliveries have been made against it by default; earlier "
        "refusals leave it retryable. A lapsed recipe cannot be resumed: "
        "re-issue this call for a fresh one. When ``parsed_metadata`` is "
        "omitted, the source filename's stem is the title and the vault's "
        "filename parsing still runs. An entry's ``sha256`` binds that leg of "
        "an upload recipe, so the upload endpoint refuses any other bytes for "
        "that leg; pass it again on the completion entry."
    ),
)
...
_INFER_EDGES = model_param(
    ...
    "infer_edges",
    mcp=(
        "Tier 1 edges land as production edges; Tier 2 candidates are staged "
        "for review via ``list_staging_edges``. A Tier-1 ``supersedes`` edge "
        "transitions its target as part of edge execution, with no "
        "``update_lifecycles`` call, per the vault's lifecycle table. A target "
        "already in "
        "a landing state gets the edge and no write. A target in any other "
        "state is not superseded: no edge is created, ``edges_dropped`` "
        "advances, and ``summary.edge_warnings`` carries "
        "``supersede_target_not_transitionable`` (``supersede_target_missing`` "
        "or ``supersede_target_read_failed`` for an absent or unreadable "
        "target). A chain repair never leaves fewer supersedes edges than it "
        "found: a replacement that is refused or fails on write "
        "(``edge_creation_failed``) withholds its removals under "
        "``chain_repair_withheld``. Also reported rather than raised: "
        "``lifecycle_transition_failed``, with the edge standing, and "
        "``chunk_lifecycle_sync_failed``. If any existing edge in a candidate "
        "version chain has a non-``version_chain`` rationale, the group's "
        "Tier-1 adds are downgraded to staged Tier-2 and no auto-transition "
        "fires, so the outcome depends on the vault's prior edge graph."
    ),
)
...
_DRY_RUN = Field(
    description=(
        ...
        "says. The summary comes back with every count at zero, an entry in "
        "``previews`` for each file that would succeed and one in ``errors`` "
        "for each that would be refused -- together accounting for the "
        "batch. Each preview names the resolved doc_type, "
        ...
        "``transfer_token`` is read but not spent. Where an upload recipe comes "
        "back instead, each leg's file is first checked by the "
