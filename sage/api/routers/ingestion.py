...
@router.post(
    ...
    responses={
        ...
        400: boundary_400(
            ...
            "carries `extension` (null when the source has none), "
            "`registered_source_types`, and `registered_extensions`, which maps "
            "each registered source type to the extensions it is inferred "
            "from.\n\n"
        ...
        409: {
            ...
            "description": (
                ...
                "`force_reingest_pin_mismatch`: `force: true` and "
                "`document_id` names a document that does not carry the "
                "delivered content hash, or no document at all. A pin may "
                "select any document holding these bytes, never a record "
                "holding other bytes, and is refused whether or not another "
                "document holds them. Detail carries `document_id`, "
                "`pinned_source_content_hash` (null when no document has that "
                "id), `source_content_hash`, and `existing_document_id` (null "
                "when no document holds the hash).\n\n"
                "`lifecycle_state_not_applicable`: `force: true` would change "
                "the existing document's `doc_type` while it holds a lifecycle "
                "state whose `doc_types` excludes the new doc_type; transition "
                "it to a state the new doc_type holds first.\n\n"
