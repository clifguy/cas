...
def rrf_fuse(
    ...
    ``matched_chunk_count`` carries how many distinct sections the document
    contributed across both lists, so collapsing them does not discard the
    reranking signal the count is there to give. A section is identified by the
    row's ``section_index`` -- several passages of a section the index divided
    count once, and two sections rendering the same heading path count twice --
    and by its heading path only on a row whose binding does not report one.
    Document-level rows are left out of that tally and out of the count carried
    across: they are not passages (CAS-ADR-049 Decision 5), and a tally falling
    back to the heading path would otherwise count one as a passage of its own,
    since it carries an empty path. The count carried across is the largest any row reported, not
        ...
        for rank, result in enumerate(source):
            ...
            if not result.is_document_surface:
                passages[doc_id].add(
                    ...
                    else ("heading_path", result.heading_path)
                )
