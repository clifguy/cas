...
class StackTransferConfig(BaseModel):
    ...
    token_ttl_seconds: int = Field(
        ...
        description=(
            ...
            "(mint, byte delivery, completion). Short-lived, and the default "
            "is set by what bounds it: a token is single-use, direction- and "
            "vault-scoped and size-capped, so the window governs staging "
            "retention rather than guessability, and it is wide enough that a "
            "caller stalled between recipe and byte delivery does not lose the "
            "transfer. A lapsed token is not "
