...
class RetrievalFilters(BaseModel):
    ...
    tier3_metadata: dict | None = Field(
        default=None,
        description=(
            "Tier 3 (per-doc_type typed metadata) filter, applied in SQL. "
            "Each key/value pair in the dict is matched against the "
            "document's `tier3_metadata` dict via exact equality, compared "
            "by JSON type: a string matches a stored string, and a number, "
            "integer, or boolean matches a stored value of that same JSON "
            "type. Numbers compare by value rather than by rendered text, "
            "so a stored 1.0 matches a filter of 1. A value of null matches "
            "documents whose stored field is either null or absent from the "
            "tier3_metadata dict. All pairs AND together; an empty dict is "
...
class ReabstractStartedResponse(BaseModel):
    status: Literal["reabstract_started"] = Field(
        description=(
            "Fixed discriminator confirming the re-abstraction job was accepted "
            "and dispatched. Always 'reabstract_started'."
        )
    )
    document_id: DocumentIdStr = Field(
        description="Identifier of the document whose abstract is being regenerated."
    )
    dispatched_at: datetime = Field(
        description=(
            "UTC timestamp at which the background re-abstraction job was enqueued. "
            "The abstract is regenerated by that job, not by the call that returned "
            "this response; wait for a terminal pipeline_status on the document to "
...
class SourceFileRestoreRequest(BaseModel):
    source: str = Field(
        description=(
            ...
            "calling environment's conventions, so a Windows drive-letter or "
            "UNC spelling earns an upload recipe rather than a refusal; where "
            "the two are co-located the server is the reader and its own "
            "conventions apply. Unlike an ingest, a relative path has no "
            "vault-relative reading here and is refused either way."
        )
    )
    document_id: DocumentIdStr | None = Field(
        default=None,
