...
class BatchIngestFileError(BaseModel):
    ...
    code: str | None = Field(
        ...
        description=(
            ...
            "request from its source, source type, parsed metadata and declared "
            "digest alone, "
...
class UploadRecipe(BaseModel):
    ...
    expires_at: datetime = Field(
        description=(
            ...
            "earliest leg's expiry, so no leg lapses before it. The whole "
            "exchange (every leg's byte delivery plus the completion call) "
            "must finish before this instant; after it, re-issue the "
            "originating call to mint fresh tokens."
        )
    )
    max_bytes: int = Field(
        description=(
